...
import { fetchLessonComments, postComment } from '@/lib/api/learn';
...
export function LessonComments({ lessonId }: { lessonId: string }) {
  ...
  useEffect(() => {
    fetchLessonComments(lessonId).then(setComments);
  }, [lessonId]);

  async function onSubmit(values: { content: string }) {
    const comment = await postComment({ content: values.content, lessonId });
  ...
  return (
    <div className="mt-6">
      <h3 className="mb-4 text-sm font-bold text-white">الأسئلة والتعليقات</h3>
